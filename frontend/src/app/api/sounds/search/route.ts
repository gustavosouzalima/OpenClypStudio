...
function applySongsFilters({
	...
	params.append("filter", "duration:[20.0 TO 900.0]");
	params.append("filter", `avg_rating:[${min_rating} TO *]`);
	...
	params.append(
		"filter",
		"tag:music OR tag:song OR tag:loop OR tag:beat OR tag:cinematic OR tag:background-music OR tag:instrumental",
	);
}
...
export async function GET(request: NextRequest) {
	try {
		...
		const missingFreesoundKey =
			!webEnv.FREESOUND_API_KEY ||
			webEnv.FREESOUND_API_KEY === "dev-key" ||
			webEnv.FREESOUND_API_KEY === "your_api_key_here";

		if (missingFreesoundKey) {
			return NextResponse.json(
				buildEmptyResponse({
					query,
					type,
