...
/**
 * Renderer type preference - can be switched via feature flag or settings
 */
type RendererType = "canvas" | "pixi";

function getRendererType(): RendererType {
	// Check for feature flag or setting preference
	// Default to pixi for better performance
	if (typeof window !== "undefined" && (window as any).PIXI_RENDERER_PREFER === "false") {
		return "canvas";
	}
	return "pixi";
}
...
export class RendererManager {
	...
	/**
	 * Create a renderer instance based on the renderer type preference
	 */
	private createRenderer({ width, height, fps }: { width: number; height: number; fps: number }) {
		const rendererType = getRendererType();

		if (rendererType === "pixi") {
			return new PixiRenderer({ width, height, fps });
		}

		return new CanvasRenderer({ width, height, fps });
	}
	...
	async saveSnapshot(): Promise<{ success: boolean; error?: string }> {
		try {
			...
			if ("dispose" in renderer) {
				(renderer as PixiRenderer).dispose();
			}
