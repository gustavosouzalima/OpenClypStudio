...
export const extractTimelineAudio = async ({
	...
	console.log("[extractTimelineAudio] Starting extraction:", {
	...
	if (totalDuration === 0) {
		console.warn("[extractTimelineAudio] Total duration is 0, returning silent audio");
	...
	console.log("[extractTimelineAudio] Collected audio sources:", {
		sourceCount: audioMixSources.length,
		sources: audioMixSources.map((s) => ({
			name: s.file.name,
			duration: s.duration,
		})),
	});
	...
	if (audioMixSources.length === 0) {
		console.warn("[extractTimelineAudio] No audio sources found, returning silent audio");
	...
	for (let i = 0; i < audioMixSources.length; i++) {
		...
		try {
			...
			console.error(
				`[extractTimelineAudio] Failed to process audio source ${source.file.name}:`,
				error,
			);
		}
	}
	...
	const result = createWavBlob({ samples: interleavedSamples });
	console.log("[extractTimelineAudio] Extraction complete:", {
		blobSize: result.size,
		expectedSamples: totalSamples * NUM_CHANNELS,
	});
