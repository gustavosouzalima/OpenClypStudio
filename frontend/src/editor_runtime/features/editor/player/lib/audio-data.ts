...
export class AudioDataManager {
  ...
  private async loadAudioData(src: string, id: string): Promise<void> {
    try {
      ...
      // Some browser/playable MP4 variants fail in media-utils parser
      // (e.g. "avcC, hvcC or VPX not found"). Playback still works via Html5Video,
      // so skip waveform extraction for this item instead of breaking runtime.
      if (/avcC,\s*hvcC\s*or\s*VPX\s*not\s*found/i.test(message)) {
        console.warn(
          `Unsupported container metadata for waveform at ${src}, skipping`
        );
        return;
      }

      // Waveform extraction must never break playback/editor runtime.
      console.warn(`Waveform extraction failed for ${src}:`, message);
      return;
    }
  }
