...
export const generateCaption = (
  ...
  const displayFromMs =
    options.displayFrom + captionLine.start * 1000 - sourceOffsetMs + syncOffsetMs;
  const displayToMs =
    options.displayFrom + captionLine.end * 1000 - sourceOffsetMs + syncOffsetMs;
  ...
  const caption = {
    ...
    name: "Caption",
    display: {
      from: Math.max(0, displayFromMs),
      to: Math.max(0, displayToMs)
    },
    metadata: {
      sourceUrl,
    ...
    details: {
      ...
      color: options.textColor || "#DADADA",
