...
function SongsView() {
	const MUSIC_CATEGORIES = [
		...
		{ id: "background", label: "Background", query: "background music cinematic" },
		{ id: "beats", label: "Beats", query: "beat instrumental groove" },
		{ id: "ambient", label: "Ambient", query: "ambient pad atmospheric" },
		{ id: "loops", label: "Loops", query: "music loop rhythm loop" },
		{ id: "piano", label: "Piano", query: "piano instrumental emotional" },
	];

	const [activeCategory, setActiveCategory] =
		useState<(typeof MUSIC_CATEGORIES)[number]["id"]>("all");
	...
	useEffect(() => {
		...
		const activeCategoryEntry =
			MUSIC_CATEGORIES.find((category) => category.id === activeCategory) ??
			MUSIC_CATEGORIES[0];
		const effectiveQuery = [activeCategoryEntry.query, query.trim()]
			.filter(Boolean)
			.join(" ")
			.trim();
		...
		const timeoutId = setTimeout(async () => {
			try {
				...
				const params = new URLSearchParams({
					type: "songs",
					page_size: "40",
					sort: effectiveQuery ? "score" : "downloads",
				});
	...
	const playSong = ({ sound }: { sound: SoundEffect }) => {
		if (playingId === sound.id) {
			audioElement?.pause();
			setPlayingId(null);
			return;
		...
		if (sound.previewUrl) {
			...
			audio.play().catch((playError) => {
				console.error("Failed to play music preview:", playError);
				setPlayingId(null);
			});
			setAudioElement(audio);
			setPlayingId(sound.id);
	...
	return (
		<div className="mt-1 flex h-full flex-col gap-5">
			<div className="flex items-center gap-3">
				<Input
					placeholder="Search songs and music loops"
					className="w-full"
					containerClassName="w-full"
					value={query}
						...
						{!isLoading &&
							...
							songs.map((song) => (
								<AudioItem
									...
									metaBadge={
										song.tags.find((tag) =>
											["music", "loop", "ambient", "beat", "instrumental"].includes(
												tag.toLowerCase(),
											),
										) ?? "music"
									}
...
function AudioItem({ sound, isPlaying, onPlay, metaBadge }: AudioItemProps) {
